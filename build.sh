...
## Check for CI
	...
	then
		...
		export SERVER_URL="https://cloud.drone.io/CannedShroud/${DRONE_REPO_NAME}/${KBUILD_BUILD_VERSION}/1/2"
	else
...
#		git clone --depth=1 https://github.com/mvaisakh/gcc-arm64 $KERNEL_DIR/gcc64
#		git clone --depth=1 https://github.com/mvaisakh/gcc-arm $KERNEL_DIR/gcc32
	...
	then
		msg "|| Cloning Clang-14 ||"
#		git clone --depth=1 https://gitlab.com/ElectroPerf/atom-x-clang $KERNEL_DIR/clang
		# Toolchain Directory defaults to clang
		TC_DIR=$KERNEL_DIR/clang
...
setversioning() {
	# For staging branch
	KERNELNAME="Atom-X-$LINUXVER-$DEVICE-$(TZ=Asia/Kolkata date +"%Y-%m-%d-%s")"
...
build_kernel() {
	...
	if [ "$PTTG" = 1 ]
 	then
            tg_post_msg "<b>🔨 Atom-X Kernel Build Triggered</b>
...
gen_zip() {
	...
	sed -i "s/kernel.for=.*/kernel.for=DailyUse/g" anykernel.sh
	sed -i "s/kernel.compiler=.*/kernel.compiler=$KBUILD_COMPILER_STRING/g" anykernel.sh
	sed -i "s/kernel.made=.*/kernel.made=CannedShroud/g" anykernel.sh
	sed -i "s/kernel.version=.*/kernel.version=$LINUXVER/g" anykernel.sh
	sed -i "s/message.word=.*/message.word=Thank You for choosing Atom-X kernel./g" anykernel.sh
